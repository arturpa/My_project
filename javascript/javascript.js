...
//URLvideo = new Array();
//URLvideo[0] = 'QOac7t4kSUI';
//URLvideo[1] = 'jk5r1mP6om8';
...
function onPlayerStateChange(event)
{
	...
	{
		players.forEach(stopOtherPlayers, event);
	}
}
		
function stopOtherPlayers(element, index, array)
{
	var triggeredID;
	var currentID;
	triggeredID = this.target.getIframe().id.trim();
	currentID = element.getIframe().id.trim();
	if
	(
		triggeredID != currentID 
		&& this.target.getPlayerState() == YT.PlayerState.PLAYING
		&& element.getPlayerState() == YT.PlayerState.PLAYING
	)
	{
		element.pauseVideo();
	}
...
jQuery("#wowslider-container1").wowSlider({effect:"book",prev:"",next:"",duration:20*100,delay:20*100,width:480,height:360,autoPlay:true,autoPlayVideo:false,playPause:false,stopOnHover:true,loop:false,bullets:1,caption:false,captionEffect:"none",controls:true,controlsThumb:false,responsive:1,fullScreen:false,gestures:2,onBeforeStep:0,images:0});
